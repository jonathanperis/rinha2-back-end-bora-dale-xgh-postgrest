...
export function debitos() {
  ...
  const url = `${baseUrl}/clientes/transacoes`;
  const res = http.post(url, payload, {
    headers: { 'Content-Type': 'application/json' },
    tags: { endpoint: '/clientes/transacoes' },
  });
...
export function validacoes() {
  ...
  group('Validações cliente', () => {
    ...
    url = `${baseUrl}/clientes/transacoes`;
    // POST a credit transaction.
    res = http.post(
      url,
      JSON.stringify({ cliente_id: randomClienteId(), valor: 1, tipo: 'c', descricao: 'toma' }),
      {
        headers: { 'Content-Type': 'application/json' },
        tags: { endpoint: '/clientes/transacoes' },
      }
    ...
    res = http.post(
      url,
      JSON.stringify({ cliente_id: randomClienteId(), valor: 1, tipo: 'd', descricao: 'devolve' }),
      {
        headers: { 'Content-Type': 'application/json' },
        tags: { endpoint: '/clientes/transacoes' },
      }
    ...
    const invalidRequests = [
      { cliente_id: 9999, valor: 1.2, tipo: 'd', descricao: 'devolve', expectedStatus: 422 },
      { cliente_id: 9998, valor: 1, tipo: 'x', descricao: 'devolve', expectedStatus: 422 },
      { cliente_id: 9997, valor: 1, tipo: 'c', descricao: '123456789 e mais', expectedStatus: 422 },
      { cliente_id: 9996, valor: 1, tipo: 'c', descricao: '', expectedStatus: 422 },
      { cliente_id: 9995, valor: 1, tipo: 'c', descricao: null, expectedStatus: 422 },
    ];
